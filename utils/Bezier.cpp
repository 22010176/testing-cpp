...
void DrawBezierCurf(SDL_Renderer* renderer, vector<Vec2> Points, float dx) {
	SDL_SetRenderDrawColor(renderer, 255, 255, 0, 255);
	int len = (int)Points.size();
	if (len < 2) return;

	vector<Vec2> Cof = GetBezierCo(Points);
	float x0 = Points[0].x, y0 = Points[0].y, x = 0, y = 0;
	for (float i = 0; i <= 1.f + dx / 2; i += dx) {
		x = y = 0;
		for (int j = 0; j < len; j++) {
			Vec2 a = Cof[j];
			x += a.x * powf(i, j) * powf(1 - i, (float)len - 1 - j);
			y += a.y * powf(i, j) * powf(1 - i, (float)len - 1 - j);
		}
